...
def compile_script_source():
    if sys.platform.startswith('win'):
        import RoundCreator.compile_win
        return RoundCreator.compile_win.source
    import RoundCreator.compile_linux
    return RoundCreator.compile_linux.source

def test_script_source():
    if sys.platform.startswith('win'):
        import RoundCreator.test_win
        return RoundCreator.test_win.source
    import RoundCreator.test_linux
    return RoundCreator.test_linux.source

def template_source():
    import RoundCreator.cpp_template
    return RoundCreator.cpp_template.source

def hightail_XML_config(contest_path):
    import RoundCreator.hightail_config
    return RoundCreator.hightail_config.source%contest_path
